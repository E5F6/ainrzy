        ...
        <section className="w-full flex flex-col justify-center items-center gap-5 px-7 py-20">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 justify-center items-stretch gap-5 mt-3">
                {roles.map((role , index) => (
                    <div key={index} className="flex flex-col justify-center items-stretch gap-2 rounded-lg border-2 border-sky-600 max-w-[350px] overflow-hidden text-center">
                        ...
                        <Link href={role.url} className="bg-sky-600 hover:bg-sky-700 transition ease-in-out px-3 py-2 font-medium text-sm mb-0 text-slate-200 rounded-full">{role.button}</Link>
                        </div>
                    </div>
                ))}
            </div>
