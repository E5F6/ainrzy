        ...
        <section className="w-full flex justify-center items-center px-7 py-20 sextion">
            ...
            <div className="grid grid-cols-1 justify-center items-stretch gap-5 mt-3">
                {steps.map((step , index) => (
                    <div key={index} className="flex flex-col justify-center items-stretch gap-2 rounded-lg border-2 px-5 py-3 border-sky-600 overflow-hidden text-center">
                        <h1 className="text-2xl font-semibold text-slate-200 mt-0 self-start">{step.name}</h1>
                        <h2  className="text-xl font-medium text-slate-300 my-auto self-start">{step.desc}</h2>
                        <Link href={step.url} className="underline font-medium text-sm mb-0 text-slate-200 self-start">{step.link}</Link>
                    </div>
                ))}
            </div>
            </div>
